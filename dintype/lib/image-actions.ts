...
// New function to save a generated image to the user's collection
export async function saveGeneratedImage(imageData: {
  ...
  try {
    ...
    if (!imageData.image_url || !imageData.prompt) {
      return {
        success: false,
        error: "Missing required image data (prompt or image_url)",
      }
    }

    // Insert the image into the generated_images table
    const { data, error } = await supabaseAdmin
      .from("generated_images")
      .insert({
        user_id: userId,
        ...
        tags: imageData.tags || [],
        collection_id: imageData.collection_id || null,
        favorite: false,
      })
      .select()
      .single()
