...
const imageOptions = [
  { value: "1", label: "1 Bild", tokens: 0 },
  { value: "4", label: "4 Bilder", tokens: 20 },
  { value: "6", label: "6 Bilder", tokens: 30 },
  { value: "8", label: "8 Bilder", tokens: 40 },
]
      ...
      {!isMounted ? (
        ...
        <>
            ...
            <div className={`${isMobile ? 'mb-4' : 'mb-6'}`} key="suggestions-container">
              ...
              <div className="min-h-[100px]" key="suggestions-stable-wrapper">
                {categories.length > 0 ? (
                  ...
                  <div key="suggestions-empty" className="text-center py-8 text-muted-foreground">Inga förslagskategorier tillgängliga.</div>
                )}
              </div>
            </div>
                ...
                <button
                  onClick={() => {
                    navigator.clipboard.readText().then((text) => {
                      setPrompt(text)
                      toast({ title: "Klistrat in från urklipp" })
                    })
                  }}
                  className="p-2 hover:bg-gray-600 rounded transition-colors"
                  title="Klistra in"
                ...
                {generatedImages.length > 0 && (
                  <div className={`flex ${isMobile ? 'flex-col gap-1' : 'gap-2'}`} key="action-buttons">
                    <Button variant="outline" size="sm" onClick={handleDownloadAll} className={isMobile ? 'text-xs px-2 py-1' : ''}>
                      ...
                      {isMobile ? 'Ladda ner alla' : 'Ladda ner alla'}
                    </Button>
                    <Button
                      variant="outline"
                      size="sm"
                    ...
                    >
                      ...
                      {isMobile ? 'Spara alla' : 'Spara alla'}
                    </Button>
                    <Button variant="outline" size="sm" onClick={() => router.push("/samlingar")} className={isMobile ? 'text-xs px-2 py-1' : ''}>
            ...
            <div className="min-h-[400px]" key="results-container">
              {isGenerating ? (
                <div key="generating-state-media" className={`flex flex-col items-center justify-center ${isMobile ? 'h-[50vh]' : 'h-[70vh]'} text-center`}>
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold mb-2">Skapar ditt mästerverk</h3>
                  <p className="text-muted-foreground max-w-md px-4 mb-4">
                    Vår AI målar din vision. Detta tar vanligtvis 10-30 sekunder.
                  </p>
                  ...
                  <p className="text-xs font-medium text-primary uppercase tracking-tighter tabular-nums">
                    Generering pågår: {Math.round(generationProgress)}%
                  </p>
                  ...
                  {timeoutWarning && (
                    <div key="gen-timeout-warning-wrapper" className="mt-6 flex items-center gap-2 text-amber-500 bg-amber-500/10 px-3 py-1.5 rounded-full border border-amber-500/20" suppressHydrationWarning>
                      ...
                      <span className="text-xs font-semibold uppercase tracking-tight">Tar längre tid än väntat...</span>
                    </div>
                  )}
                </div>
                ...
                <div key="empty-state-media-wrapper" className={`flex flex-col items-center justify-center ${isMobile ? 'h-[50vh]' : 'h-[70vh]'} text-center`}>
                  ...
                  <h3 className="text-xl sm:text-2xl font-bold mb-2">Redo att skapa?</h3>
                  <p className="text-muted-foreground max-w-sm px-4">
                    Ange en beskrivning till vänster och klicka på "Generera" för att se din fantasi komma till liv.
                  </p>
                </div>
              ) : generatedImages.length > 0 ? (
                        ...
                        {characterId ? (
                          <>
                            ...
                            <Button
                              ...
                              title={likedImageUrls.has(image) ? "Ta bort gilla" : "Gilla"}
                            >
          ...
          <PremiumUpgradeModal
            ...
            description="Uppgradera till Premium för att generera obegränsat med bilder."
          ...
          <PremiumUpgradeModal
            ...
            feature="Tokens Slut"
            description="Du har använt dina 100 gratis premium-tokens. Köp fler tokens för att använda premiumfunktioner"
