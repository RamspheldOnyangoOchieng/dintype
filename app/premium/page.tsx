...
export default function PremiumPage() {
  ...
  const handleTokenPurchase = async () => {
    ...
    if (!selectedPackage) {
      toast.error("Selected token package could not be found")
      return
    }
    ...
    if (user.isAdmin) {
      try {
        ...
        const data = await response.json()
        if (response.ok) {
          toast.success(`Administrator! You have added ${selectedPackage.tokens} tokens to your account.`)
    ...
    if (creditBalance >= selectedPackage.price) {
      try {
        ...
        const data = await response.json()
        if (response.ok) {
          toast.success(`Success! You have purchased ${selectedPackage.tokens} tokens.`)
          setTokenBalance(prev => prev + selectedPackage.tokens)
          setCreditBalance(prev => prev - selectedPackage.price)
          await refreshUser()
        } else {
          throw new Error(data.error || "Failed to convert credits")
        }
      } catch (error: any) {
        toast.error(error.message)
      } finally {
      ...
      toast.error("Not enough credits. Your subscription gives you monthly credits.")
    }
  }
